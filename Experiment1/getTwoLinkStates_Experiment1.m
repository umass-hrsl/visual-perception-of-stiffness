function getTwoLinkStates_Experiment1_MEGHAN()
        ...
        for trial = 1:length(trialConditions) %for all 6 simulted joint stiffnesses
            ...
            switch(experimentConditions{experiment})
                case 'Original'
                    [tAdjusted{trial}] = t;
                case 'Constant'
                    [tAdjusted{trial}] = getTimeSeries(t, state, arm, gainConstant, 0);
                case 'Reverse'
                    [tAdjusted{trial}] = getTimeSeries(t, state, arm, K_reverse(trial), -1/3);
                case 'Variable'
                    [tAdjusted{trial}] = getTimeSeriesVariable(state0, arm, circle, control, gainConstant, 0);
            end
...
function [tAdjusted] = getTimeSeries(t, state, arm, K, b)
...
function [tAdjusted] = getTimeSeriesVariable(state0, arm, circle, control, K, b)
    % determine Kq from trial condition
