function runTwoLinkExperiment_Experiment2()
    %subject identifier for a single experiment that is ran
    %this value should be changed for each subject.
    ...
    % load pre-computed simulation produced from getTwoLinkStates_Experiment2
    load('twoLinkStates_Experiment2.mat')
    ...
    trialConditions = [randperm(4) randperm(4) randperm(4) randperm(4) randperm(4)]; %block randomized order (5 blocks) of each of the four trial conditions

    % generate variable for storing ratings
    setappdata(fig, 'ratings', zeros(1, length(trialConditions)));
    
    for trial = 1:length(trialConditions) %loop through the 20 different trial conditions
...
function startTrial(hObject, eventdata)
    ...
    set(handles.startButton, 'Visible', 'off', 'String', sprintf('Start Trial %d of 20', 1+getappdata(fig, 'trialNumber')));
end
