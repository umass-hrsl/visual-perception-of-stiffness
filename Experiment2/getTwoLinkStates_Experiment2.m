function getTwoLinkStates_Experiment2()
    %function used to produce a mat file for the simulations used in Experiement 2
    close all
    clear all
    ...
    trialConditions = {'Original','Constant','Reverse','Variable'};
    
    %velocity gain for Reverse condition
    K = [11.5 10.3 7.2 7.0 6.9 6.9];
    gainConstant = 5.4; %Used when converting time series for the constant and variable conditions
    
    for trial = 1:length(trialConditions) %for all 4 conditions
        
        % determine Kq from trial condition
        control.Kq = getKq(3, 'elbow'); %elbow, shoulder, both %The value 3 is used as it gets the elbow condition of 30 Nm/rad
        
        % simulate the arm motion for 65 seconds
        [t,state] = ode45(@(t,y) updateState(t,y,arm, circle, control),[0:.001:65],state0);
        
        % get rid of the first 5 seconds (to remove transients)
        remove = find(t<5);
        t(remove) = [];
        state(remove, :) = [];
        t = t-t(1);
        ...
        switch(trialConditions{trial})
            case 'Original' 
                [tAdjusted{trial}] = t;
            case 'Constant'
                [tAdjusted{trial}] = getTimeSeriesConstant(t, state, arm, circle, control, gainConstant);
            case 'Reverse'
                [tAdjusted{trial}] = getTimeSeriesReverse(t, state, arm, circle, control, K(4));
            case 'Variable'
                [tAdjusted{trial}] = getTimeSeriesVariable(t, state, state0, arm, circle, control, gainConstant);
        end
    end
    
    %save the structure to be run in the actual experiment
    save('twoLinkStates_Experiment2.mat', 'twoLinkStates', 'tAdjusted', 'arm', 'circle', 'control', 'state0');
